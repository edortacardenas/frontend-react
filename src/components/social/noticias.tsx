...
const Noticias: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-slate-50 min-h-screen">
      <header className="flex flex-col sm:flex-row justify-between items-center mb-8 pb-4 border-b border-slate-200">
      ...
      {isLoading && articles.length === 0 && (
        <div className="flex flex-col justify-center items-center h-64 text-slate-600">
      ...
      {error && articles.length === 0 && !isLoading && (
         <Alert variant="destructive" className="mb-6 bg-red-50 border-red-300 text-red-700">
      ...
      {articles.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {articles.map((article, index) => (
            <Card key={article.url + index} className="flex flex-col bg-white overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out rounded-lg">
              {article.urlToImage ? (
                <img 
              ...
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-semibold leading-tight text-slate-800 hover:text-blue-600 transition-colors">
              ...
              <CardFooter>
                <Button asChild variant="link" className="p-0 h-auto text-blue-600 hover:text-blue-800 font-medium">
      ...
      {articles.length > 0 && hasMoreNews && (
        <div className="text-center mt-10">
          <Button onClick={handleLoadMore} disabled={isLoadingMore} size="lg" className="bg-blue-600 hover:bg-blue-700 text-white">
            {isLoadingMore ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Cargando más...</> : 'Mas noticias'}
          </Button>
        </div>
      )}
      {!hasMoreNews && articles.length > 0 && (
        <p className="text-center text-slate-500 mt-10 py-4 bg-slate-100 rounded-md">No hay más noticias para mostrar.</p>
      )}
    </div>
  );
};
